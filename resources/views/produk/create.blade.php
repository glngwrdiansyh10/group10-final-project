...
@section('page-subtitle', 'Tambah produk baru ke master data')
...
<div class="max-w-2xl">
    ...
    <div class="card">
        <h2 class="font-bold text-base mb-5 pb-4 border-b border-dark-400">
            <i class="fas fa-box-open text-emerald-400 mr-2"></i>Data Produk Baru
        </h2>

        <form method="POST" action="{{ route('owner.produk.store') }}" class="space-y-4">
            @csrf
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input id="kode_produk" name="kode_produk" type="text"
                        ...
                        value="{{ old('kode_produk') }}" placeholder="cth: PRD-001">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input id="kategori" name="kategori" type="text"
                        class="form-input @error('kategori') border-red-500 @enderror"
                        value="{{ old('kategori') }}"
                        placeholder="cth: Minuman, Snack, Sembako"
                        list="kategori-list">
                    <datalist id="kategori-list">
                        @foreach($kategori as $k)<option value="{{ $k }}">@endforeach
                    </datalist>
                    @error('kategori') <p class="form-error"><i class="fas fa-circle-exclamation"></i> {{ $message }}</p> @enderror
                </div>
                <div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input id="harga_beli" name="harga_beli" type="number" min="0"
                        class="form-input @error('harga_beli') border-red-500 @enderror"
                        value="{{ old('harga_beli', 0) }}" placeholder="0">
                    @error('harga_beli') <p class="form-error"><i class="fas fa-circle-exclamation"></i> {{ $message }}</p> @enderror
                </div>
                <div>
                    ...
                    <input id="harga_jual" name="harga_jual" type="number" min="0"
                        class="form-input @error('harga_jual') border-red-500 @enderror"
                        value="{{ old('harga_jual', 0) }}" placeholder="0">
                    @error('harga_jual') <p class="form-error"><i class="fas fa-circle-exclamation"></i> {{ $message }}</p> @enderror
                </div>
            </div>

            <div class="max-w-xs">
                <label class="form-label" for="stok_minimum">
                    Batas Stok Minimum <span class="text-red-400">*</span>
                    <span class="text-slate-500 font-normal ml-1">(untuk alert stok kritis)</span>
                </label>
                <input id="stok_minimum" name="stok_minimum" type="number" min="0"
                    class="form-input" value="{{ old('stok_minimum', 5) }}">
            </div>
