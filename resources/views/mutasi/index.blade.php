...
<div class="flex flex-wrap items-center justify-between gap-3 mb-5">
    <form method="GET" class="flex flex-wrap items-center gap-2">
        ...
        @if(request()->anyFilled(['tanggal','tipe']))
            <a href="{{ route('gudang.mutasi.index') }}" class="btn-secondary btn-sm"><i class="fas fa-xmark"></i></a>
        @endif
    </form>
    <div class="flex gap-2">
        <a href="{{ route('gudang.dashboard') }}" class="btn-secondary">
            <i class="fas fa-boxes-stacked"></i> Stok Saat Ini
        </a>
            ...
            <tbody>
                ...
                <tr class="table-tr-hover">
                    <td class="table-td text-xs text-slate-400">
                        <p class="font-semibold text-slate-200">{{ $m->created_at->format('d/m/Y') }}</p>
                    ...
                    <td class="table-td text-center">
                        @if ($m->tipe === 'masuk')
                            <span class="badge-green"><i class="fas fa-arrow-down mr-1"></i> Masuk</span>
                        @else
                            <span class="badge-red"><i class="fas fa-arrow-up mr-1"></i> Keluar</span>
                        @endif
                    </td>
                    <td class="table-td text-center font-bold text-base">
                        <span class="{{ $m->tipe === 'masuk' ? 'text-emerald-400' : 'text-red-400' }}">
